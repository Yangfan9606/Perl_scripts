...
Program : intersectBed 的overlap 结果输出overlap len 和 rate,  输入2个bed的chr列即可
...
while(<IN>){
	...
	my @P = ($s1,$s2,$e1,$e2);
#	@P = sort {$a <=> $b} @P;
#	$peak{"$c1\t$P[0]\t$P[-1]"}{$of} = "$info1\t$info2\t$strand\t$L";
#	$Lake{"$c1\t$P[0]\t$P[-1]"}{fc} .= "$of;";
#	print OUT "$c1\t$P[0]\t$P[-1]\t$info1\t$info2\t$strand\t$L\n";
	print OUT "$L\t$f1\t$f2\t$c1,$s1,$e1\t$c2,$s2,$e2\t$info1\t$info2\n";
}
close IN;
#foreach my $k(sort {$a cmp $b} keys %peak){
#	my @F = split/;/,$Lake{$k}{fc};
#	@F = sort {$b <=> $a} @F;
#	print OUT "$k\t$peak{$k}{$F[0]}\t$F[0]\n";
#	print OUT "$k\t$peak{$k}{$F[0]}\n";
#	last;
#}
#############
